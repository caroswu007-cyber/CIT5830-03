...
# alchemy_url = f"https://eth-mainnet.g.alchemy.com/v2/{alchemy_token}"
# infura_url = f"https://mainnet.infura.io/v3/{infura_token}"

def connect_to_eth():
    url = "https://eth-mainnet.g.alchemy.com/v2/rKN3ChGNmvKkUj9P4JmDC/getNFTs/?owner=vitalik.eth"
...
def is_ordered_block(w3, block_num):
    ...
    def priority_fee(tx):
        ...
        if base_fee is None:
            return int(gp) if gp is not None else 0

        is_type2 = (tx_type == 2) or (mp is not None or mf is not None)
        if is_type2 and mp is not None and mf is not None:
            return max(0, min(int(mp), int(mf) - int(base_fee)))

        if gp is None:
            return 0
        return max(0, int(gp) - int(base_fee))

    fees = [priority_fee(tx) for tx in txs]
...
def get_contract_values(contract, admin_address, owner_address):
    ...
    has_role = contract.functions.hasRole(
        default_admin_role, Web3.to_checksum_address(admin_address)
    ).call()
